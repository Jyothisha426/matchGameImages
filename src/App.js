...
class App extends Component {
  ...
  getRandomImage = () => {
    ...
    const randomImage = imagesList[randomNum]
    return randomImage
  }
  ...
  onClickPlayAgain = () => {
    this.setState({isGameGoingOn: true, score: 0, timer: 60})
    this.timerID = setInterval(() => this.tick(), 1000)
    this.renderImagesCard()
  }
  ...
  onClickImage = clickedId => {
    const {mainImage} = this.state
    const {id: activeId} = mainImage
    ...
    if (clickedId === activeId) {
      ...
      this.setState({isGameGoingOn: false})
      clearInterval(this.timerID)
    }
  }
  ...
  renderImagesCard = () => {
    const {activeTabId, mainImage} = this.state
    const filteredImages = imagesList.filter(
      eachImage => eachImage.category === activeTabId,
    )
    return (
      <div className="second-container">
        ...
        <ul className="images-container">
          {filteredImages.map(eachImage => (
            <li className="each-image" key={eachImage.id}>
              <button
                type="button"
                className="image-btn"
  ...
  render() {
    const {score, isGameGoingOn, timer} = this.state
          ...
          <div className="score-container">
            ...
            <div className="timer-container">
              <img
                ...
                className="timer"
              />
              <p className="score-points">{timer} sec</p>
            </div>
          </div>
        </nav>

        <div className="game-container">
          {isGameGoingOn ? this.renderImagesCard() : this.renderResultCard()}
        </div>
      </div>
    )
  }
